...
def start_actor():
    ...
    log(f"Actor futás elindítva, run_id: {run_id}")
    return run_id
...
def wait_for_finish(run_id):
    while True:
        ...
        if r.status_code != 200:
            log(f"Hiba a run lekérésénél: {r.text}")
            time.sleep(5)
            continue
...
def get_latest_post(dataset_id):
    ...
    r = requests.get(url)
    if r.status_code != 200:
        log(f"Hiba a dataset lekérésénél: {r.text}")
        return None
...
run_id = start_actor()
if not run_id:
    log("Nem sikerült elindítani a scraper-t.")
    exit()

dataset_id = wait_for_finish(run_id)
if not dataset_id:
    log("Nem sikerült datasetet lekérni.")
    exit()
